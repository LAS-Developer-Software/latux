...
import { customStartPageHtml } from 'vs/workbench/browser/customStartPage';
...
export function openCustomStartPage(): void {
	const existing = document.getElementById('custom-start-page-overlay');
	if (existing) {
		return;
	}

	const overlay = document.createElement('div');
	overlay.id = 'custom-start-page-overlay';
	overlay.style.position = 'fixed';
	overlay.style.inset = '0';
	overlay.style.zIndex = '1000';
	overlay.style.background = 'white';
	overlay.style.overflow = 'hidden';
	overlay.innerHTML = customStartPageHtml;
