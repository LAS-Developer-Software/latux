...
// Register Overleaf command with button
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'workbench.action.openOverleaf',
			title: localize('openOverleaf', 'Open Overleaf'),
	...
	run(accessor: ServicesAccessor) {
		...
		openerService.open('https://www.overleaf.com');
	}
});
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'myBrowser.openOverleaf',
			title: 'Overleaf öffnen',
			category: 'Help',
			icon: Codicon.globe,
			f1: true,
			menu: {
	...
	run() {
		window.open('https://www.overleaf.com', '_blank', 'noopener,noreferrer');
	}
});
...
// The built-in welcome page is intentionally disabled so the custom startup
// experience provided by workbench.ts can take over.
